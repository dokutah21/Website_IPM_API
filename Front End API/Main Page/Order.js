...
async function renderOrder(listproduct){
    ...
    var html =  listproduct.map(item=>{
        ...
        <div class="cart-item">
            <span><button onclick=DeleteOrder(${order["maHoaDon"]})><i class="far fa-times-circle"></i></button></span>
            <img src="${item["anhSach"]}" alt="">
            <div class="content">
                <h3><b>${item["tenSach"]}</b></h3>
                <div class="qty">Số lượng: ${order["list_json_ChiTietHoaDon"][0]["soLuong"]}</div>
                <div class="price">${totalPrice}₫</div>
            </div>
        </div>
        `
    })
    $(".listcart").html(html.join(""));
};
